...
(define-library (schema)
  (export new open delete!
          ...
          fixed-header-size disk position description)
  ...
  (begin
    ...
    (define fixed-header-size (* 2 disk:block-ptr-size))    ; next, previous; variable # of occupancy bits to be added dynamically
    ...
    ; HULPFUNCTIE
    (define (tag->symbol tag)
      (cond ((= tag natural-tag) 'natural)
            ((= tag integer-tag) 'integer)
            ((= tag decimal-tag) 'decimal)
            ((= tag string-tag)  'string)
            (else (error "Foute invoer van type" tag))))

    (define (description scma)
      (define count (nr-of-attributes scma))
      (let loop ((index 0))
        (if (= index count)
            '()
            (let* ((typ-tag (type scma index))
                   (typ-sym (tag->symbol typ-tag))
                   (siz-val (size scma index)))
              ; bouwt de lijst op ((type size) ...)
              (cons (list typ-sym siz-val) 
                    (loop (+ index 1)))))))
